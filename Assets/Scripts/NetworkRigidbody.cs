...
{
	double m_InterpolationBackTime = 0.0;
	double m_ExtrapolationLimit = 1.5;
	...
	void OnSerializeNetworkView(BitStream stream, NetworkMessageInfo info)
	{
		...
		if (stream.isWriting)
		{
			...
			stream.Serialize(ref turnspeed);
	...
	void FixedUpdate()
	{
		//SINCE INTERPOLATIONBACKTIME IS 0, WE WILL NEVER INTERPOLATE, ONLY EXTRAPOLATE
		...
		else
		{
			...
			if (extrapolationLength < m_ExtrapolationLimit)
			{
				...
				Quaternion addedRotation = Quaternion.Euler(0.0f, latest.turnspeed * extrapolationLength, 0.0f);
				Quaternion preferedrot = latest.rot * addedRotation;
